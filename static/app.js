// Function to show a change on dropdown menu
function optionChanged(selectedID){

    // Check that value is selected in dropdown
    console.log(selectedID);
 
    // Read the json file for the data that we loaded
    d3.json("data/samples.json").then((data) => {
 
   //  console.log(data);
 
    // resets the dropdown menu
    d3.select("#selectedDataset").html("");   
    
    // Select the metadata array and for each item append the item ID and adds ID to dropdown
    data.metadata.forEach(item =>
         {
          // console.log(item.id);
         d3.select ("#selectedDataset").append('option').attr('value', item.id).text(item.id);
         });
    // Selected which value is passed
    d3.select("#selectedDataset").node().value = selectedID;
    
    // Filter the Metadata for selected ID from dropdown menu
 ...
 d3.select("#selectedDataset").on('change',() => {
